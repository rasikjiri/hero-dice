create or replace function public.connect_online_player_and_set_ready(
  p_session_id uuid,
  p_player_id text,
  p_device_id text
)
...
revoke all on function public.connect_online_player_and_set_ready(uuid, text, text) from public;
grant execute on function public.connect_online_player_and_set_ready(uuid, text, text) to anon, authenticated;

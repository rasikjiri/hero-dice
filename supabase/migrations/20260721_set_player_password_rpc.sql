...
begin
  if coalesce(length(trim(p_new_password)), 0) < 6 then
    raise exception using message = 'Heslo musí mít alespoň 6 znaků.';
  end if;
  ...
  if v_admin_player_id is null then
    raise exception using message = 'Neplatná nebo neaktivní admin session.';
  end if;
  ...
  if coalesce(v_is_admin, false) = false then
    raise exception using message = 'Pouze admin může měnit hesla hráčů.';
  end if;
  ...
  if not exists (
    ...
    raise exception using message = 'Hráč nebyl nalezen.';
  end if;
